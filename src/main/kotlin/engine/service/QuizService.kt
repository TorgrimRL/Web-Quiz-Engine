...
class QuizService(
  ...
  fun getSampleQuiz(): QuizQuestionResponse =
    ...
    val saved =
        quizRepository.save(
            Quiz(
                    ...
                    answer = postQuizRequest.answer ?: emptyList())
  ...
  fun proccesAnswer(id: Int, answer: QuizAnswerRequest, email: String): QuizResponse {
    val quiz =
        quizRepository.findByIdOrNull(id)
            ?: throw ResponseStatusException(HttpStatus.NOT_FOUND, "Did not find quiz")
  ...
  fun getUserSpecificCompletedQuizzesPage(page: Int, email: String): Page<QuizCompletion> {
    val currentUser =
        userRepository.findUserByEmail(email)
            ?: throw ResponseStatusException(HttpStatus.UNAUTHORIZED)
    val pageResult =
        quizCompletionsRepository.findQuizCompletionByUserOrderByCompletedAtDesc(
